...
public interface CustomJpaRepository<T, ID extends Serializable>
     ...
     * Без @Query щоб Spring Data JPA використовував реалізацію з CustomJpaRepositoryImpl.
     */
    Optional<T> softDelete(@Param("id") ID id, EntityGraph entityGraph);
    
    /**
     * Знаходить всі активні сутності.
     * Використовує @Query щоб Spring Data JPA не намагався створити query автоматично.
     * 
    ...
    /**
     * М'яке видалення сутності (встановлює active = false) - перевантажений метод без EntityGraph.
     * Реалізація в CustomJpaRepositoryImpl.
     */
     ...
     * Реалізація в CustomJpaRepositoryImpl.
     * @Query потрібен щоб Spring Data JPA не намагався створити query автоматично.
     * Використовує SELECT щоб отримати кількість, потім виконує softDelete в реалізації.
     */
